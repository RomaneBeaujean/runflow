<template>
  <div v-if="!hasWarmup" class="flex justify-center p-2">
    <Button size="small" outlined icon="pi pi-plus" label="Echauffement" @click="handleAddWarmup"></Button>
  </div>
  ...
  <draggable v-model="internalSteps" :group="{ name: 'steps', pull: true, put: true }" item-key="id"
    ...
    <template #item="{ element: step, index: idx }">
      <div class="relative">
        ...
        <WorkoutFormWorkStep v-if="step.type == 'work'" :step="step"
          @update:step="(s: WorkoutStep) => handleUpdateStep(s, idx)" />
        <WorkoutFormRecoveryStep v-if="step.type == 'recovery'" :step="step"
          @update:step="(s: WorkoutStep) => handleUpdateStep(s, idx)" />
        <WorkoutFormWarmupStep v-if="step.type == 'warmup'" :step="step"
          @update:step="(s: WorkoutStep) => handleUpdateStep(s, idx)" />
      </div>
    </template>
  </draggable>
  ...
  <div class="flex mt-3 justify-center mb-2 gap-2">
    <Button size="small" outlined icon="pi pi-plus" label="Exercice" @click="handleAddWorkStep"></Button>
    <Button size="small" outlined icon="pi pi-plus" label="Récupération" @click="handleAddRecoveryStep"></Button>
  </div>

</template>

<script setup lang="ts">
import { createWorkoutStep } from '@/domain/factories/WorkoutFactory';
import { WorkoutStep, WorkoutWithSteps } from '@/domain/types/Workout';
import Icon from '@/ui/components/Icon.vue';
import WorkoutFormRecoveryStep from '@/ui/pages/training-plan/training-plan/workouts/workout/workout-form/WorkoutFormRecoveryStep.vue';
import WorkoutFormWarmupStep from '@/ui/pages/training-plan/training-plan/workouts/workout/workout-form/WorkoutFormWarmupStep.vue';
import WorkoutFormWorkStep from '@/ui/pages/training-plan/training-plan/workouts/workout/workout-form/WorkoutFormWorkStep.vue';
...
const handleUpdateStep = (step: WorkoutStep, index: number) => {
